...
if isfield(param, 'initsol')
    ...
    xsol = zeros(Ny, Nx);%At(zeros(No, 1));
end
...
for t = 1:param.max_iter    
   ...
   for q = 1:R
        ...
        [proj{q}, no_sub_itr{q}] = solver_proj_elipse_fb(1 ./ pU{q} .* v2{q}, residual_data_block, y{q}, pU{q}, epsilont{q}, proj{q}, elipse_proj_max_iter, elipse_proj_min_iter, elipse_proj_eps);
            v2{q} = v2{q} + pU{q} .* (residual_data_block -  proj{q});%v2{q} = v2{q} + pU{q} .* r2_ - pU{q} .* proj{q};
         ...
         for q = 1:R
           ...
           elseif   ((t>t_update{q}+param.adapt_bound_steps) || (t_update{q} == param.max_iter )) && ...
               ...
               count_eps_update = count_eps_update +1;
                ...
                if param.verbose>=1
                    fprintf('========================================================\n')
                    fprintf('Updated bound of block %0.0f, %0.0f times, down to = %0.1f\n', q, count_eps_update, epsilont{q});
                    fprintf('========================================================\n')
           ...
           if (block_l2norm{q} > epsilonts{q})  && ...
               ...
               count_eps_update = count_eps_update +1;
                ...
                if param.verbose>=1
                    fprintf('=========================================================\n')
                    fprintf('Updated bound of block %0.0f, %0.0f times, up to = %0.1f\n', q, count_eps_update, epsilont{q});
                    fprintf('=========================================================\n')
    ...
    g1 = (sigma2(1) ./sigma1(1))*At(uu);%zeros(size(xsol));  
    clear uu;
    for k = 1:P
  ...
  if (param.use_reweight_steps && t == reweight_steps(reweight_step_count)) || ...
       ...
       rel_sol_norm_change < param.reweight_rel_obj)
        ...
        for k = 1:P
            ...
            weights{k} = reweight_alpha(k) ./ (reweight_alpha(k) + (d_val./d_val_max));
            weights{k}(d_val > d_val_max * param.reweight_abs_of_max) = 0;
        end
