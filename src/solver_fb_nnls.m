function [sol, L2_v, sol_v, snr_v] = solver_fb_nnls(y, A, At, param)
...
% skew_v = zeros(param.max_iter, 1);
% kurt_v = zeros(param.max_iter, 1);
...
while 1
    ...
    res = y - A(xhat);
    grad = At(res);
%     kurt_v(iter) =kurtosis(real(grad(:)));
%     skew_v(iter) =skewness(real(grad(:)));
    
%     if param.verbose >= 1
%         fprintf('  Kurtosis = %e, Skewness = %e\n', ...
%             kurt_v(iter), skew_v(iter));
%     end
    % Update variables
    iter = iter + 1;
    prev_obj = curr_obj;
